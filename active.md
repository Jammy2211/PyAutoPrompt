...
- session: claude --resume "datacube-hannah-preset"
- status: library-shipped, workspace-pending
- worktree: ~/Code/PyAutoLabs-wt/datacube-hannah-preset
- library-pr: https://github.com/PyAutoLabs/PyAutoArray/pull/311
- repos:
  - PyAutoArray: feature/datacube-hannah-preset
  - autolens_workspace_developer: feature/datacube-hannah-preset
- summary: |
    Add a "hannah" instrument preset to the jax_profiling datacube + interferometer
    delaunay profilers that pins Hannah Stacey's real ALMA settings (n_channels=34,
    n_visibilities=16984, pixel_scale=0.125", shape_native=(40, 40), mask_radius=2.3").
    Library side (PyAutoArray): extend `Interferometer.from_fits` to accept
    `raise_error_dft_visibilities_limit` so callers profiling the JAX-traceable DFT
    path at ALMA-scale can bypass the >10k-visibility safety guard. Workspace side:
    promote `mask_radius` to a per-instrument field in the INSTRUMENTS dict, gate
    the full-pipeline cube JIT (Part C) behind `CUBE_FULL_JIT=1` since lower+compile
    at n_channels=34 is ~70s, pin per-channel regression literal `-204838.07924622478`,
    write the simulated dataset + result artifacts.
