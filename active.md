...
- session: claude --resume "mesh-geometry-picklable"
- status: library-dev
- worktree: ~/Code/PyAutoLabs-wt/mesh-geometry-picklable
- repos:
  - PyAutoArray: feature/mesh-geometry-picklable
- note: |
    Carve-out from viz-subprocess-feasibility (#1279) Q2. The Phase 4
    feasibility spike found FitImaging is unpicklable because
    AbstractMeshGeometry stores `self._xp = xp` as a module. Fix:
    replace with `self._use_jax: bool` + `_xp` as property — same
    pattern as Analysis._xp (PyAutoFit) and AbstractMaker._xp
    (PyAutoArray decorators). ~10 lines on one class + pickle round-trip
    test. End-to-end verified: FitImaging round-trips through pickle
    with bit-exact log_likelihood preservation (4.6 MB / fit).

    File-disjoint with in-flight knn-barycentric (#317) on PyAutoArray
    (KNN edits inversion/mesh/{interpolator,mesh}/ and plot/*; we edit
    inversion/mesh/mesh_geometry/abstract.py + new test file).
