...
- status: library-dev
- worktree: ~/Code/PyAutoLabs-wt/critical-curves-linewidth
- repos:
  - PyAutoArray: feature/critical-curves-linewidth
- summary: |
    User-reported visual tweak — critical curves and caustics overlays
    were too thick to inspect underlying lens-model results. Reduces the
    ...
    exclusively by critical curves and caustics, so the change targets
    exactly the reported overlays with no incidental side-effects. No
    public API touched; no config files involved (the only mat_wrap YAML
    with these settings is the legacy z_projects/subhalo config which is
    not loaded on the active code path). Runs in parallel with
    knn-barycentric on PyAutoArray — distinct branches, disjoint file sets.
