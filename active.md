...
- session: claude --resume "nss-checkpointing-and-visualization"
- status: library-dev
...
- summary: |
    ...
    loop in NSS._fit (replacing the single run_nested_sampling call) so
    we can hook checkpoint writes + analysis.visualize() between outer
    iterations. New `checkpoint_interval` kwarg; iterations_per_quick_update
    becomes functional (Phase 1 left it as a no-op log).

    Key finding from Phase 1 follow-up: nss.ns.run_nested_sampling's outer
    loop is plain Python, not jax.lax.while_loop — so no upstream
    yallup/nss PR is needed. Both phases ship in a single PyAutoFit PR.
