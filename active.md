...
- summary: |
    ...
    math fix for Pmax > 1 (Delaunay barycentric). Refactor
    InterferometerSparseOperator.curvature_matrix_via_sparse_operator_from
    to mirror ImagingSparseOperator.curvature_matrix_diag_from(rows, cols,
    vals, *, S) using mapper.sparse_triplets_curvature. Convert the existing
    raise-test into a parity test (rtol=1e-4 vs the mapping path), and update
    the one Pmax=1 call-site in
    autolens_workspace_test/scripts/jax_assertions/sparse_operators.py to use
    the new signature.

    Affected repos:
      - PyAutoArray (library, primary)
      - autolens_workspace_test (workspace follow-up — one call-site update)

    Tracking issue #314 already exists; planned.md entry deleted in same
    push that adds this block.

## nss-search-wrapper
- issue: https://github.com/PyAutoLabs/PyAutoFit/issues/1271
- session: claude --resume "nss-search-wrapper"
- status: library-dev
...
- summary: |
    ...
    PyAutoFit/autofit/non_linear/search/nest/nss/ with search.py +
    samples.py + __init__.py mirroring Nautilus. JAX-traceable likelihood +
    prior closures using Phase 0's xp=jnp plumbing. Optional-import guard
    so `import autofit` works without nss. Verification via new
    autolens_workspace_developer/searches_minimal/nss_first_class.py
    against the HST MGE einstein_radius=1.5996 baseline. Phase 2
    (checkpointing) and Phase 3 (quick-update visualization) stubbed —
    kwargs accepted with warnings, full wiring deferred.
